...
function initChartFilters() {
    ...
    function updateChartWithYear(year) {
        ...
        // Criar um novo gráfico com os dados do ano selecionado
        const ctx = document.getElementById('productionChart').getContext('2d');
        
        const config = {
            type: 'line',
            data: {
                labels: [...productionDataByYear[year].labels],
                datasets: productionDataByYear[year].datasets.map(dataset => ({
                    ...dataset,
                    data: [...dataset.data]
                }))
            },
        ...
        console.log('Labels:', productionChart.data.labels.length);
        console.log('Datasets:', productionChart.data.datasets.length);
        console.log('Dados atualizados:', productionChart.data.datasets.map(d => ({
            cultura: d.label,
            dados: d.data
        })));
    }
    ...
    btn2023.addEventListener('click', function(e) {
        e.preventDefault();
    ...
    // Definir estado inicial (2023 como padrão)
    btn2023.classList.add('active');
    btn2024.classList.remove('active');
    updateChartWithYear('2023');
}
